...
from model.base import TextToWaveform
from model.helper import clip_grad_value_, slice_segments
from module.vits_losses import DiscriminatorLoss, FeatureMatchingLoss, GeneratorLoss, KlLoss
from module.vits_modules import MultiPeriodDiscriminator
from torchdata.data import DistributedBucketSampler
from torchdata.data_type import AudioCodec, PretrainedLM
from model.utils import extensive_process_batch as process_batch
...
class VitsModel(TextToWaveform):
    ...
    def forward(self, tokens, speakers=None, noise_scale=1, length_scale=1, noise_scale_w=1.0, max_len=1000,
            ref_spec = None, ref_spec_lens = None, ref_codec = None, ref_codec_lens = None):
        ...
        audio_pred, attn, y_mask, (z, z_p, m_p, logs_p) = self.net_g.infer(
            ...
            ref_codec=ref_codec,
            ref_codec_lens=ref_codec_lens
        )
        return audio_pred, attn, y_mask, (z, z_p, m_p, logs_p)

    def training_step(self, batch, batch_idx):
        speakers, audio_codec, audio_codec_lens = None, None, None
        if AudioCodec in self._train_dl.dataset.sup_data_types_set or PretrainedLM in self._train_dl.dataset.sup_data_types_set:
            ...
            speakers = batch_dict.get("speaker_id", None)
            audio_codec = batch_dict.get("audio_codec", None)
            audio_codec_lens = batch_dict.get("audio_codec_lens", None)
        ...
        ref_spec, ref_spec_lens = None, None
        ref_codec, ref_codec_lens = None, None
        if audio_codec is not None:
            if audio_codec.size(1) == 8:  # codec-only
                ref_spec, ref_spec_lens = audio_codec, audio_codec_lens
            else:  # multi modal ref input
                ref_codec, ref_codec_lens = audio_codec, audio_codec_lens
        else:  # mel-only
            ref_spec, ref_spec_lens = spec, spec_lengths

        with autocast(enabled=True):
            audio_pred, l_length, attn, ids_slice, text_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q) = self.net_g(
                text, text_len, spec, spec_lengths, speakers, ref_spec, ref_spec_lens, ref_codec, ref_codec_lens,
            )
    ...
    def validation_step(self, batch, batch_idx):
        speakers, audio_codec, audio_codec_lens = None, None, None
        if AudioCodec in self._validation_dl.dataset.sup_data_types_set:
            batch_dict = process_batch(batch, self._train_dl.dataset.sup_data_types_set)
            audio = batch_dict.get("audio")
            audio_len = batch_dict.get("audio_lens")
            text = batch_dict.get("text")
        ...
        if audio_codec is not None:
            ...
            ref_spec, ref_spec_lens = self.audio_to_melspec_processor(audio, audio_len, linear_spec=True)

        audio_pred, _, mask, *_ = self.net_g.infer(text, text_len, speakers, max_len=1000,
                                                   ref_spec=ref_spec, ref_spec_lens=ref_spec_lens,
                                                   ref_codec=ref_codec, ref_codec_lens=ref_codec_lens)
        ...
        # valid_mel_loss = F.l1_loss(mel.detach(), audio_pred_mel.detach()).detach()
        # metrics = {
        #     'val_mel_loss': valid_mel_loss
        # }
        # self.log_dict(metrics, on_step=True, sync_dist=True)
        ...
        if batch_idx == 0 and isinstance(self.logger, WandbLogger) and HAVE_WANDB:
            ...
            audios += [
                wandb.Audio(
                    ...
                    caption=f"val_wav_target_1",
                    sample_rate=self._cfg.sample_rate,
                ),
                wandb.Audio(
                    audio_pred[0, : audio_pred_len[0]].data.cpu().to(torch.float).numpy(),
                    caption=f"val_wav_predicted_1",
                    sample_rate=self._cfg.sample_rate,
                ),
                wandb.Audio(
                    audio[3, : audio_len[3]].data.cpu().to(torch.float).numpy(),
                    caption=f"val_wav_target_2",
                    sample_rate=self._cfg.sample_rate,
                ),
                wandb.Audio(
                    audio_pred[3, : audio_pred_len[3]].data.cpu().to(torch.float).numpy(),
                    caption=f"val_wav_predicted_2",
                    sample_rate=self._cfg.sample_rate,
                ),
                wandb.Audio(
                    audio[4, : audio_len[4]].data.cpu().to(torch.float).numpy(),
                    caption=f"val_wav_target_3",
                    sample_rate=self._cfg.sample_rate,
                ),
                wandb.Audio(
                    audio_pred[4, : audio_pred_len[4]].data.cpu().to(torch.float).numpy(),
                    caption=f"val_wav_predicted_3",
                    sample_rate=self._cfg.sample_rate,
                ),
            ]
    ...
    def _loader(self, cfg, use_sampler):
        try:
        ...
        if use_sampler:  # use sampler only in training
            sampler = DistributedBucketSampler(dataset, **self.cfg.train_ds.batch_sampler)

            dataloader = torch.utils.data.DataLoader(  # noqa
                dataset=dataset, collate_fn=dataset.collate_fn, batch_sampler=sampler, **cfg.dataloader_params,
            )
        else:
            dataloader = torch.utils.data.DataLoader(  # noqa
...
#
    ...
    def setup_training_data(self, cfg):
        self._train_dl = self._loader(cfg, use_sampler=False)

    def setup_validation_data(self, cfg):
        self._validation_dl = self._loader(cfg, use_sampler=False)
